...
def define_model_log_prob(model, model_loss, x, y, params_flattened_list, params_shape_list, tau_list, tau_out, normalizing_const=1., predict=False, prior_scale = 1.0, device = 'cpu'):
    ...
    def log_prob_func(params):
        ...
        #     # print(dist.log_prob(w))
        #     l_prior = dist.log_prob(w).sum() + l_prior
        #     i_prev += index
        l_prior = - torch.sum(tau_list * params**2) / 2
        ...
        if model_loss != "regression_intermediate" and model_loss != "regression_intermediate":
    ...
    #         crit = nn.BCEWithLogitsLoss(reduction='sum')
            ...
            # beta = 5.
            # warn(f"working with beta = {beta}")
            # ll = - 1 *(crit(beta*output, y_device.long().view(-1))) 
        ...
        elif model_loss == 'regression_intermediate':
            W = params_unflattened[0]
            b_W = params_unflattened[1]
            a = params_unflattened[2]
            b_a = params_unflattened[3]
            Z = params_unflattened[4]
            Phi = params_unflattened[5]

            b_W = b_W.reshape([b_W.size()[0],1])
            b_a = b_a.reshape([b_a.size()[0],1])

            ll = - 0.5 * tau_out * ( (Z - W@x_device.T - b_W)**2 + (Phi - model.sigma(Z))**2 + (y_device - a@Phi - b_a)**2 ).sum()

        elif model_loss == 'multi_class_linear_output_intermediate':
            if predict:
                output = fmodel(x_device, params=params_unflattened)
                crit = nn.CrossEntropyLoss(reduction='sum')
                ll = - tau_out *(crit(output, y_device.long().view(-1)))
                ...
                ll = - 10* tau_out * crit(y_net.T, y_device.long().view(-1)) - 0.5 * tau_out * ( (Z - W@x_device.T - b_W)**2 + (Phi - model.sigma(Z))**2 ).sum() - 0.5 * tau_out * ((y_net - a@Phi - b_a)**2).sum()
