...
dyn_output_file_name="dyn_MALA_cnn_mnist_D%.1e_lr%.1e"%(Delta.item(),learning_rate.item())+"_"+identifier+".txt"
...
output_file_name="MALA_cnn_mnist_%s.txt"%identifier
...
X_train_mnist=torch.tensor(file_mnist['x_train'])
y_train_mnist=torch.tensor(file_mnist['y_train'])
X_test_mnist=torch.tensor(file_mnist['x_test'])
y_test_mnist=torch.tensor(file_mnist['y_test'])

X_train_mnist=X_train_mnist.reshape([60000,1,28,28])[:n]/255
X_test_mnist=X_test_mnist.reshape([10000,1,28,28])[:n_test]/255
y_test=y_test_mnist[:n_test]
y_train=y_train_mnist[:n]

mean_X=torch.mean(X_train_mnist)
std_X=torch.std(X_train_mnist)
X_train=(X_train_mnist-mean_X)/std_X
X_test=(X_test_mnist-mean_X)/std_X
####
if(verbose):
...
#define network
lambda_dict={'conv_layer.weight': torch.tensor(16),'conv_layer.bias': torch.tensor(16),'fc.weight': torch.tensor(72),'fc.bias':torch.tensor(72)} #dictionary with the priors
std=1e-4
std_dict={'conv_layer.weight':std,'conv_layer.bias':std,'fc.weight': std, 'fc.bias':std} #initialization standard deviations
student= ConvNeuralNet(num_classes=10,informed_weights_dict=None,std_weight_dict=std_dict)
loss = nn.CrossEntropyLoss(reduction='sum')
