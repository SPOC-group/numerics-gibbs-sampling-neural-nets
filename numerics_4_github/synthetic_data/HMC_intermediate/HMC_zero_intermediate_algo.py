from HMC_sampler_intermediate_algo import sampler
...
def main():
    ...
    params = {"num_leapfrog_steps":1000, "step_size":0.00005}
    sampler_HMC = sampler(num_samples=num_samples, spacing=10, delta=delta)
    ...
    np.savetxt(f"data/intermediate_algo/{algo}_{init}_delta_{delta}_train_{id}.csv", train_mse, delimiter=",")
    np.savetxt(f"data/intermediate_algo/{algo}_{init}_delta_{delta}_test_{id}.csv", test_mse, delimiter=",")
