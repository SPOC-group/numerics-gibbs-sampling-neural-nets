...
class sampler:
    ...
    def logP_unnorm(self, *current_state):
        #current_state=(W,b_W, a,b_a)
        W=current_state[0]
        b_W=current_state[1]
        a=current_state[2]
        b_a=current_state[3]
        logP=-0.5*(tf.reduce_sum((self.y-self.sigma(self.X@tf.transpose(W) + b_W)@tf.transpose(a) - b_a)**2)/self.Delta_y) - tf.norm(W)**2*self.lambda_W_0/2 - tf.norm(a)**2*self.lambda_a_0/2 - tf.norm(b_W)**2*self.lambda_W_0/2- tf.norm(b_a)**2*self.lambda_a_0/2 
        #print(logP)
        return logP
    ...
    def initialise_weights(self, initialisation):
        ...
        if self.initialisation == "Zero":
            ...
            b_W=tf.random.normal([1,self.K], 0, 1e-4)
            b_a=tf.random.normal([1], 0, 1e-4)
        ...
        self.initial_state=(W,b_W, a,b_a)
    ...
    def make_observables(self):
        ...
        for t in range(self.num_samples):
            ...
            a=self.samples_result[2][t]
            b_a=self.samples_result[3][t]
